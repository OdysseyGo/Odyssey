...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for notif in pending_notifications:
            ...
            self.stdout.write(f"Successfully sent: {notif.title} -> {notif.user.username}")
        ...
        old_notifications = Notification.objects.filter(created_at__lt=cleanup_threshold)
        count, _ = old_notifications.delete()
        
        if count > 0:
            self.stdout.write(self.style.SUCCESS(f"Cleaned up {count} old notifications (older than 7 days)."))